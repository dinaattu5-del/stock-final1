...
const defaultCategories: Category[] = [
  { id: '1', name: 'Produits', description: 'Produits généraux' },
  { id: '2', name: 'Services', description: 'Services proposés' },
  { id: '3', name: 'Matériel', description: 'Matériel et équipements' }
];
  ...
  useEffect(() => {
    ...
    const ordersChannel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, (payload) => {
        ...
        dispatch((prevState) => ({
          type: 'SET_SALES',
          payload: [payload.new, ...prevState.sales],
        }));
  ...
  const clearPendingOrders = async () => {
    ...
    try {
      const { error } = await supabase
        .from('orders')
        .update({ status: 'acknowledged' }) // Update status to 'acknowledged'
